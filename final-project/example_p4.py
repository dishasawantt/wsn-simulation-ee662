"""
Part 4: Configurable Network Parameters

Builds on Part 3, adding:
1. MAX_CLUSTER_MEMBERS: Limit on nodes per cluster
   - When a cluster reaches max capacity, JOIN_REQUESTs are forwarded to form new clusters
   
2. TX_POWER per cluster:
   - Each cluster can have its own transmission power (affects range)
   - Option for uniform power across all nodes
   - Power must be between TX_POWER_MIN and TX_POWER_MAX

3. PACKET_LOSS_RATE:
   - Simulates imperfect channel conditions
   - Configurable loss rate for control and data packets
   - Optional distance-dependent loss

Per instructions.txt Step 4:
a. Number of child nodes allowed for a cluster
b. TxPower of each cluster (TxMin to TxMax)
c. Packet loss ratio
"""
...
import math
from source import config_p4 as config
...
print(f"Part 4: Configurable Network Parameters + TCP-Like Reliability")
print(f"Nodes: {config.SIM_NODE_COUNT}, ROOT: Node {ROOT_ID}")
print(f"Max cluster members: {config.MAX_CLUSTER_MEMBERS if config.MAX_CLUSTER_MEMBERS else 'Unlimited'}")
print(f"TX Power: {'Uniform' if config.USE_UNIFORM_TX_POWER else 'Per-cluster'} ({config.TX_POWER_MIN}-{config.TX_POWER_MAX})")
print(f"Packet loss rate: {config.PACKET_LOSS_RATE*100:.1f}%")
print(f"TCP-Like Reliability: Retransmit={config.ENABLE_RETRANSMISSION}, ACK={config.ENABLE_ACK}, Multipath={config.ENABLE_MULTIPATH_ROUTING}")
