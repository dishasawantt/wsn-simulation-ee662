...
Builds on Part 6, adding:
1. CLUSTER MERGING (7a):
   - Small clusters (< MIN_CLUSTER_SIZE) merge with neighbors
   - Reduces total number of clusters in the network
   
2. LOAD BALANCING (7b):
   - Distribute members evenly across clusters
   - Transfer members from overloaded to underloaded clusters
   
3. ENERGY-AWARE CH ROTATION (7c):
   - Rotate CH role based on energy levels
   - Prolongs network lifetime by distributing CH burden

Per instructions.txt Step 7:
a. Minimize the number of clusters
b. Minimize energy consumed in the network (per packet energy consumption)
"""
...
import math
from source import config_p7 as config
...
OVERLAP_STATS = {
    'ch_requests_rejected': 0,
    'router_nodes_created': 0,
    'ch_handoffs_initiated': 0,
    'ch_handoffs_completed': 0,
    'ch_handoffs_failed': 0
}

CH_POSITIONS = {}

# ============== PART 6: FAILURE & RECOVERY STATS ==============
FAILURE_STATS = {
    'nodes_killed': 0,
    'nodes_recovered': 0,
    'orphans_detected': 0,
...
# Event log: list of (timestamp, event_type, node_id, details)
EVENT_LOG = []

# Track dead nodes: {node_id: {'killed_at': time, 'recover_at': time, 'previous_role': role}}
DEAD_NODES = {}

# Track orphan nodes: {node_id: {'orphaned_at': time, 'previous_parent': gui}}
ORPHAN_NODES = {}

# ============== PART 7: CLUSTER OPTIMIZATION STATS ==============
OPTIMIZATION_STATS = {
    'clusters_merged': 0,
    'merge_attempts': 0,
    'merge_failures': 0,
    ...
    'ch_rotations': 0,
    'clusters_eliminated': 0,
    'initial_cluster_count': 0,
    'final_cluster_count': 0
}

# Track cluster sizes for optimization: {net_addr: member_count}
CLUSTER_SIZES = {}
...
def log_recovery(timestamp, node_id, recovery_time):
    """Log a node recovery event."""
    if config.LOG_NODE_RECOVERIES:
...
def log_orphan(timestamp, node_id, parent_id):
    """Log an orphan detection event."""
    if config.LOG_ORPHAN_EVENTS:
        log_event(timestamp, 'ORPHAN_DETECTED', node_id, f"Lost parent: {parent_id}")
    FAILURE_STATS['orphans_detected'] += 1
    current_orphan_count = len(ORPHAN_NODES)
    FAILURE_STATS['max_orphan_count'] = max(FAILURE_STATS['max_orphan_count'], current_orphan_count)

def log_orphan_recovered(timestamp, node_id, new_parent_id):
    """Log an orphan recovery event."""
    if config.LOG_ORPHAN_EVENTS:
...
def log_network_join(timestamp, node_id, parent_id, cluster_net):
    """Log a network join event."""
    if config.LOG_NETWORK_JOINS:
...
class SensorNode(wsn.Node):
    ...
    def set_role(self, new_role, *, recolor=True):
        ...
        if recolor:
            colors = {
                ...
                Roles.ROOT: (0, 0, 0),
                Roles.ROUTER: config.ROUTER_COLOR,
                Roles.DEAD: config.DEAD_NODE_COLOR,  # Dark red for dead nodes
            }
    ...
    def evaluate_cluster_efficiency(self):
        """Check if this cluster should merge with another (7a)."""
        if self.role not in [Roles.CLUSTER_HEAD]:
            return None
        if not config.ENABLE_CLUSTER_MERGING:
            return None
        ...
        if member_count >= config.MIN_CLUSTER_SIZE:
            return None  # Cluster is large enough
        
        # Find potential merge targets (nearby CHs with capacity)
        merge_candidates = []
        for gui, entry in self.neighbors_table.items():
            if entry.role in [Roles.CLUSTER_HEAD, Roles.ROOT]:
                if gui in ALL_NODES:
                    ...
                    capacity = config.MAX_CLUSTER_MEMBERS - target_size if config.MAX_CLUSTER_MEMBERS else 999
                    
                    # Can target absorb our members?
                    if capacity >= member_count + 1:  # +1 for ourselves
        ...
        # Select best merge target
        if config.MERGE_PREFERENCE == 'NEAREST':
    ...
    def request_cluster_merge(self, target_gui):
        """Request to merge this cluster into target cluster (7a)."""
        if self.role not in [Roles.CLUSTER_HEAD]:
            return False
        
        OPTIMIZATION_STATS['merge_attempts'] += 1
        self.log(f"Requesting merge into cluster headed by {target_gui}")
        
        pck = {
            'dest': wsn.BROADCAST_ADDR,
            'type': 'CLUSTER_MERGE_REQUEST',
            'gui': self.id,
            'source_gui': self.id,
            'dest_gui': target_gui,
            'member_count': len(self.members_table),
    ...
    def merge_cluster(self, new_ch_gui, new_ch_addr):
        """Complete cluster merge - notify members and demote (7a)."""
        self.log(f"Merging cluster into {new_ch_gui}")
        
        # Notify all members to join new cluster
        for member_gui in list(self.members_table.keys()):
            pck = {
                'dest': wsn.BROADCAST_ADDR,
                'type': 'CLUSTER_MERGE_NOTIFY',
                'gui': self.id,
                'source_gui': self.id,
                'dest_gui': member_gui,
                'new_ch_gui': new_ch_gui,
    ...
    def check_load_balance(self):
        """Check if load balancing is needed (7b)."""
        ...
        # Get sizes of neighbor clusters
        neighbor_clusters = []
        for gui, entry in self.neighbors_table.items():
        ...
        # Check if we're significantly larger
        if my_size > avg_size * config.LOAD_BALANCE_THRESHOLD:
            # Find smallest neighbor that can accept members
            neighbor_clusters.sort(key=lambda x: x['size'])
            for target in neighbor_clusters:
                size_diff = my_size - target['size']
                if size_diff >= config.LOAD_BALANCE_MIN_DIFF:
                    return target
        
        return None
    
    def transfer_members(self, target_gui, count):
        """Transfer members to another cluster for load balancing (7b)."""
        if not self.members_table:
            return
        
        # Select members to transfer (prefer those closer to target)
        ...
        for i in range(transfer_count):
            ...
            pck = {
                'dest': wsn.BROADCAST_ADDR,
                'type': 'MEMBER_TRANSFER_REQUEST',
                'gui': self.id,
                'source_gui': self.id,
                'member_gui': member_gui,
                'target_ch_gui': target_gui,
                'created_at': self.now
            }
            self.send_with_loss(pck)
        ...
        self.log(f"Transferred {transfer_count} members to {target_gui}")
    
    def check_ch_rotation(self):
        """Check if CH role should rotate to another node (7c)."""
        if self.role not in [Roles.CLUSTER_HEAD]:
            return None
        if not config.ENABLE_CH_ROTATION:
            return None
        
        # Select best candidate for new CH
        candidates = []
        for gui, member in self.members_table.items():
            if gui in ALL_NODES:
                node = ALL_NODES[gui]
        ...
        # Prefer node with more neighbors (more central)
        if config.PREFER_HIGH_DEGREE_CH:
            candidates.sort(key=lambda x: x['neighbors'], reverse=True)
        
        # Only rotate if candidate has more neighbors than current CH
        my_neighbors = len(self.neighbors_table)
        best = candidates[0]
        if best['neighbors'] > my_neighbors + 1:  # +1 to avoid frequent swaps
            return best['gui']
        
        return None
    
    def initiate_ch_rotation(self, new_ch_gui):
        """Initiate CH role rotation to optimize cluster topology (7c)."""
        if self.handoff_in_progress:
            return False
        
        self.log(f"Initiating CH rotation to {new_ch_gui} for optimization")
    ...
    def run_cluster_optimization(self):
        """Main optimization check - runs periodically on CHs (7)."""
        if self.role not in [Roles.CLUSTER_HEAD]:
            return
        ...
        # 7a: Check if we should merge with another cluster
        merge_target = self.evaluate_cluster_efficiency()
        if merge_target:
            self.request_cluster_merge(merge_target['gui'])
            return  # Don't do other optimizations if merging
        
        # 7b: Check if we should transfer members for load balancing
        balance_target = self.check_load_balance()
        if balance_target:
            self.transfer_members(balance_target['gui'], config.MEMBER_TRANSFER_COUNT)
            return
        
        # 7c: Check if we should rotate CH role
    ...
    def kill_node(self):
        """Simulate node failure - node stops all activity."""
        ...
        DEAD_NODES[self.id] = {
            'killed_at': self.now,
            'recover_at': self.now + config.FAILURE_DURATION,
            'previous_role': previous_role,
            'previous_addr': self.addr,
            'previous_ch_addr': self.ch_addr,
            'previous_parent': self.parent_gui
        }
        
        # If this was a CH, its members become orphans
        ...
        # Clear all timers
        for timer_name in ['TIMER_PROBE', 'TIMER_JOIN_REQUEST', 'TIMER_HEART_BEAT', 
                          'TIMER_DATA_PACKET', 'TIMER_ORPHAN_CHECK']:
        ...
        self.log(f"NODE KILLED - was {previous_role.name}")
        
        # Schedule recovery
        self.set_timer('TIMER_RECOVER', config.FAILURE_DURATION)
    
    def recover_node(self):
        """Recover from failure - rejoin the network."""
        if self.role != Roles.DEAD:
            return
        ...
        self.log(f"NODE RECOVERING - rejoining network")
        
        # Restart as UNDISCOVERED
        self.set_role(Roles.UNDISCOVERED)
        self.wake_up()
        self.set_timer('TIMER_PROBE', 1)
    
    def become_orphan(self, dead_parent_gui):
        """Called when this node's parent dies."""
        if self.role == Roles.DEAD:
            return
        
        log_orphan(self.now, self.id, dead_parent_gui)
        
        ORPHAN_NODES[self.id] = {
            'orphaned_at': self.now,
            'previous_parent': dead_parent_gui
        }
        
        self.parent_gui = None
        self.log(f"ORPHANED - parent {dead_parent_gui} died")
        
        # Try to find a new parent
        if config.ENABLE_NETWORK_REPAIR:
            self.delayed_exec(config.REPAIR_DELAY, self.attempt_repair)
    
    def attempt_repair(self):
        """Attempt to rejoin the network after becoming orphan."""
        if self.id not in ORPHAN_NODES:
            return  # Already recovered
        
        self.log("Attempting network repair...")
        
        # Broadcast I_AM_ORPHAN to find new parent
        if config.BROADCAST_ORPHAN_STATUS:
            pck = {
                'dest': wsn.BROADCAST_ADDR,
                'type': 'I_AM_ORPHAN',
                'gui': self.id,
        ...
        # Also try to find a new parent from neighbors
        for gui, entry in self.neighbors_table.items():
            if entry.role in [Roles.CLUSTER_HEAD, Roles.ROOT] and gui != ORPHAN_NODES[self.id]['previous_parent']:
                # Found a potential new parent
                self.log(f"Found potential parent: {gui}")
                self.parent_gui = gui
                self.send_join_request(entry.ch_addr if entry.ch_addr else entry.address)
                return
        
        # No parent found, become unregistered and try again
        self.become_unregistered()
    
    def check_parent_alive(self):
        """Check if parent is still alive based on heartbeats."""
        ...
        if time_since_heard > config.PARENT_TIMEOUT:
            self.log(f"Parent {self.parent_gui} timeout ({time_since_heard:.1f}s)")
            return False
    ...
    def on_receive(self, pck):
        ...
        if self.role == Roles.ROOT or self.role == Roles.CLUSTER_HEAD:
            ...
            # ============== PART 7: OPTIMIZATION HANDLERS FOR CH/ROOT ==============
            elif pck_type == 'CLUSTER_MERGE_REQUEST':
                if pck.get('dest_gui') == self.id:
                    source_gui = pck.get('gui')
                    member_count = pck.get('member_count', 0)
                    current_size = len(self.members_table)
                    capacity = config.MAX_CLUSTER_MEMBERS - current_size if config.MAX_CLUSTER_MEMBERS else 999
                    
                    if capacity >= member_count + 1:
                        self.log(f"Accepting merge from {source_gui} ({member_count} members)")
                        accept_pck = {
                            'dest': wsn.BROADCAST_ADDR,
                            'type': 'CLUSTER_MERGE_ACCEPT',
                            'gui': self.id,
                            'source_gui': self.id,
                            'dest_gui': source_gui,
                            'ch_addr': self.ch_addr,
                        ...
                        self.log(f"Cannot accept merge from {source_gui} - no capacity")
            
            elif pck_type == 'CLUSTER_MERGE_ACCEPT':
                if pck.get('dest_gui') == self.id:
                    new_ch_gui = pck.get('gui')
                    new_ch_addr = pck.get('ch_addr')
                    self.kill_timer('TIMER_MERGE_TIMEOUT')
                    self.merge_cluster(new_ch_gui, new_ch_addr)
            
            # CH receives acceptance of handoff request
            ...
            if pck_type == 'HEART_BEAT':
                ...
                # PART 7: Start optimization timer for new CH
                if config.ENABLE_CLUSTER_OPTIMIZATION:
                    self.set_timer('TIMER_OPTIMIZATION', config.OPTIMIZATION_START_TIME)
            ...
            # ============== PART 5c: CH HANDOFF FOR REGISTERED NODES ==============
                ...
                if new_ch_gui == self.id:
                    ...
                    self.set_role(Roles.CLUSTER_HEAD)
                    register_ch_position(self.id)
                    self.log(f"Took over CH role from {pck['gui']} (via REGISTERED)")
                    
                    if config.ENABLE_CLUSTER_OPTIMIZATION:
                        self.set_timer('TIMER_OPTIMIZATION', config.OPTIMIZATION_INTERVAL)
        ...
        if self.role == Roles.UNREGISTERED:
            if pck_type == 'HEART_BEAT':
                ...
                if pck['dest_gui'] == self.id:
                    ...
                    if self.ch_addr is not None:
                        ...
                        # PART 6: Start orphan check timer
                        self.set_timer('TIMER_ORPHAN_CHECK', config.ORPHAN_DETECTION_INTERVAL)
                        if config.ENABLE_DATA_PACKETS:
                            self.set_timer('TIMER_DATA_PACKET', config.DATA_PACKET_START_TIME + random.uniform(0, 50))
                    
                    # PART 6: Log network join and recover from orphan status
            ...
            # PART 6: Orphan adoption
            elif pck_type == 'I_AM_ORPHAN':
                orphan_gui = pck.get('gui')
                orphan_addr = pck.get('addr')
                
                # If we're a CH/ROOT and can adopt this orphan
                if self.role in [Roles.CLUSTER_HEAD, Roles.ROOT]:
                    if not self.is_cluster_full():
                        self.log(f"Adopting orphan node {orphan_gui}")
                        # Send JOIN_REPLY to the orphan
                        if orphan_addr:
            ...
            # ============== PART 7: OPTIMIZATION PACKETS FOR REGISTERED/ROUTERS ==============
            
            elif pck_type == 'CLUSTER_MERGE_NOTIFY':
                if pck.get('dest_gui') == self.id:
                    new_ch_gui = pck.get('new_ch_gui')
                    new_ch_addr = pck.get('new_ch_addr')
                    self.log(f"Received merge notify - moving to cluster {new_ch_gui}")
                    
                    # Leave current cluster and join new one
                    self.parent_gui = new_ch_gui
                    self.set_role(Roles.UNREGISTERED)
                    if new_ch_addr:
                        self.send_join_request(new_ch_addr)
                ...
                # If I'm the member being transferred
                if member_gui == self.id:
                    self.log(f"Being transferred to cluster {target_ch_gui}")
                    self.parent_gui = target_ch_gui
                    self.set_role(Roles.UNREGISTERED)
                    
                    # Get target CH address from neighbors
    ...
    def on_timer_fired(self, name, *args, **kwargs):
        ...
        # PART 6: Recovery timer
        elif name == 'TIMER_RECOVER':
            self.recover_node()
        
        # PART 6: Orphan check timer
...
def export_event_log(path="event_log_p6.csv"):
    ...
    with open(path, 'w', newline='') as f:
        ...
        for event in EVENT_LOG:
            w.writerow([f"{event['timestamp']:.2f}", event['event_type'], 
...
def export_optimization_stats(path="optimization_stats_p7.csv"):
    """Export cluster optimization statistics."""
    with open(path, 'w', newline='') as f:
        w = csv.writer(f)
        w.writerow(['metric', 'value'])
        for key, value in OPTIMIZATION_STATS.items():
            w.writerow([key, value])
        # Add current cluster count
...
def print_metrics():
    ...
    # PART 6: Failure & Recovery stats
    print(f"\nPart 6: Node Failure & Recovery:")
    print(f"  Nodes killed: {FAILURE_STATS['nodes_killed']}")
    print(f"  Nodes recovered: {FAILURE_STATS['nodes_recovered']}")
    print(f"  Orphans detected: {FAILURE_STATS['orphans_detected']}")
    print(f"  Orphans recovered: {FAILURE_STATS['orphans_recovered']}")
    print(f"  Max simultaneous orphans: {FAILURE_STATS['max_orphan_count']}")
    if FAILURE_STATS['nodes_recovered'] > 0:
        avg_recovery = FAILURE_STATS['total_recovery_time'] / FAILURE_STATS['nodes_recovered']
        print(f"  Average recovery time: {avg_recovery:.2f}s")
    print(f"  Total events logged: {len(EVENT_LOG)}")
    
    # PART 7: Cluster Optimization stats
    print(f"\nPart 7: Cluster Optimization:")
    print(f"  Cluster merging enabled: {config.ENABLE_CLUSTER_MERGING}")
    print(f"  Load balancing enabled: {config.ENABLE_LOAD_BALANCING}")
    print(f"  CH rotation enabled: {config.ENABLE_CH_ROTATION}")
    print(f"  Merge attempts: {OPTIMIZATION_STATS['merge_attempts']}")
    print(f"  Clusters merged: {OPTIMIZATION_STATS['clusters_merged']}")
    print(f"  Merge failures: {OPTIMIZATION_STATS['merge_failures']}")
    print(f"  Clusters eliminated: {OPTIMIZATION_STATS['clusters_eliminated']}")
    print(f"  Members transferred: {OPTIMIZATION_STATS['members_transferred']}")
    print(f"  Load balance events: {OPTIMIZATION_STATS['load_balance_events']}")
    print(f"  CH rotations: {OPTIMIZATION_STATS['ch_rotations']}")
    
    # Calculate cluster reduction
    current_clusters = len([n for n in ALL_NODES.values() if n.role in [Roles.CLUSTER_HEAD, Roles.ROOT]])
...
print(f"Part 7: Cluster Optimization Protocol")
print(f"Nodes: {config.SIM_NODE_COUNT}, ROOT: Node {ROOT_ID}")
print(f"Max cluster members: {config.MAX_CLUSTER_MEMBERS if config.MAX_CLUSTER_MEMBERS else 'Unlimited'}")
print(f"TX Power: {config.TX_POWER_DEFAULT}")
print(f"Cluster Optimization: {config.ENABLE_CLUSTER_OPTIMIZATION}")
if config.ENABLE_CLUSTER_OPTIMIZATION:
    print(f"  - Cluster merging: {config.ENABLE_CLUSTER_MERGING} (min size: {config.MIN_CLUSTER_SIZE})")
    print(f"  - Load balancing: {config.ENABLE_LOAD_BALANCING}")
    print(f"  - CH rotation: {config.ENABLE_CH_ROTATION}")
print(f"Random Failures: {config.ENABLE_RANDOM_FAILURES}")
...
def schedule_random_failure():
    ...
    if not config.ENABLE_RANDOM_FAILURES:
        return
    if failure_count >= config.MAX_FAILURES:
        return
    
    # Select a random connected node to kill (not ROOT)
    eligible_nodes = [n for n in ALL_NODES.values() 
                     if n.role in [Roles.REGISTERED, Roles.CLUSTER_HEAD, Roles.ROUTER]
                     and n.id != ROOT_ID]
    ...
    if failure_count < config.MAX_FAILURES:
        sim.delayed_exec(config.FAILURE_INTERVAL, schedule_random_failure)

# Schedule first failure
if config.ENABLE_RANDOM_FAILURES:
    sim.delayed_exec(config.FAILURE_START_TIME, schedule_random_failure)
...
if config.EXPORT_METRICS:
    export_join_times("join_times_p7.csv")
    export_packet_delays("packet_delays_p7.csv")
    export_packet_traces("packet_traces_p7.csv")
    export_cluster_stats("cluster_stats_p7.csv")
    export_event_log("event_log_p7.csv")
    export_failure_stats("failure_stats_p7.csv")
    export_optimization_stats("optimization_stats_p7.csv")
    
    print(f"\nExported metrics:")
    print("  - join_times_p7.csv")
    print("  - packet_delays_p7.csv")
    print("  - packet_traces_p7.csv")
    print("  - cluster_stats_p7.csv")
    print("  - event_log_p7.csv")
    print("  - failure_stats_p7.csv")
    print("  - optimization_stats_p7.csv")
