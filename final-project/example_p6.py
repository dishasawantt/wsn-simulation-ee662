"""
Part 6: Network Recovery from Node/Link Failures

Builds on Part 5, adding:
1. RANDOM NODE FAILURES:
   - Nodes randomly killed during simulation
   - Configurable failure rate and duration
   
2. NODE RECOVERY:
   - Dead nodes recover after specified time
   - Rejoin network automatically
   
3. ORPHAN DETECTION:
   - Nodes detect when parent is dead
   - Broadcast I_AM_ORPHAN to find new parent
   
4. EVENT LOGGING:
   - Log failures, recoveries, orphans, role changes
   - Track recovery time and orphan count

Per instructions.txt Step 6:
a. Time to recover
b. Number of nodes orphan
"""
...
import math
from source import config_p6 as config
...
print(f"Part 6: Network Recovery from Node/Link Failures")
...
if config.ENABLE_RANDOM_FAILURES:
    print(f"  - Start: {config.FAILURE_START_TIME}s, Max: {config.MAX_FAILURES}, Duration: {config.FAILURE_DURATION}s")
print(f"Network Repair: {config.ENABLE_NETWORK_REPAIR}")
...
if config.EXPORT_METRICS:
    ...
    # Export Part 6 specific files
    export_event_log("event_log_p6.csv")
    export_failure_stats("failure_stats_p6.csv")
    ...
    print("  - event_log_p6.csv")
    print("  - failure_stats_p6.csv")
